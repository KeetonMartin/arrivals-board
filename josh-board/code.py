...
bullet_index = {
    ...
    "MTA": 27,
    "FE": 28,
    # Bus direction icons (for south wall mounting)
    "DIR_N": 38,  # Down arrow (uptown going away)
    "DIR_S": 39,  # Up arrow (downtown coming toward)
    "DIR_W": 40,  # Right arrow (westbound to Javits)
    # Bus routes - will use direction icons instead
    "M34+": 40,   # Westbound M34 -> right arrow
    "M34": 40,
    "M15+": 27,   # Will be overridden by direction logic
    "M15": 27,
    "BUS": 27,    # Generic fallback
    "BLANK": 37
}
...
class Arrivals:
    def __init__(self):
        # Basic setup
        self.base_url = secrets["transit_url"]  # Now storing base URL without endpoint
        ...
        # Initialize arrival queue for display
        self.arrivals_queue = [
            {
                "Line": None,
                "Arrival": 0,
    ...
    def api_call(self):
        ...
        try:
            for header in self.headers:
                # Log the endpoint we're about to call
                endpoint = header.get("endpoint", "/api/mta-arrivals")
                if not endpoint.startswith("/"):
                    endpoint = "/" + endpoint

                full_url = self.base_url + endpoint
                print(f"\nTrying endpoint: {full_url}")
                print(f"With headers: {header}")

                try:
                    data = network.fetch_data(full_url, json_path=[], headers=header)
                    if data:
                        print(f"Got data from {endpoint}:")
                        print(f"North: {len(data.get('North', []))} arrivals")
                        print(f"South: {len(data.get('South', []))} arrivals")
                        print(f"Sample data North: {data.get('North', [])[:1]}")
                        print(f"Sample data South: {data.get('South', [])[:1]}")

                        # Extend our collections with the new data
                        arrival_data["North"].extend(data.get("North", []))
                        arrival_data["South"].extend(data.get("South", []))
                        arrival_data["alerts"].extend(data.get("alerts", []))
                except Exception as e:
                    print(f"Error fetching from {endpoint}: {e}")
                    continue

            if not arrival_data["North"] and not arrival_data["South"]:
                print("No arrival data received from any endpoint")
                return None

            print("\nAfter combining and before sorting:")
            print(f"Total North arrivals: {len(arrival_data['North'])}")
            print(f"Total South arrivals: {len(arrival_data['South'])}")

            # Remove duplicates and sort
            arrival_data["North"] = self.remove_duplicates(arrival_data["North"])
            arrival_data["South"] = self.remove_duplicates(arrival_data["South"])
            arrival_data["alerts"] = self.remove_duplicate_alerts(arrival_data["alerts"])

            print("\nAfter deduplication:")
            print(f"Unique North arrivals: {len(arrival_data['North'])}")
            print(f"Unique South arrivals: {len(arrival_data['South'])}")
            ...
            print("\nFirst arrival in each direction:")
            if arrival_data["North"]: print(f"North: {arrival_data['North'][0]}")
            if arrival_data["South"]: print(f"South: {arrival_data['South'][0]}")

        except Exception as e:
            print("Main API call error:", e)
            return None
    ...
    def remove_duplicates(self, arrival_list):
        unique = []
        seen = set()
        for item in arrival_list:
            key = (item["Line"], item["Arrival"])
            if key not in seen:
                unique.append(item)
                seen.add(key)
        return unique

    def remove_duplicate_alerts(self, alerts_list):
        unique = []
        seen = set()
    ...
    def get_bus_direction_icon(self, line, ns_direction):
        """Get the appropriate direction icon for a bus arrival."""
        # M34 always goes west (to Javits) - right arrow
        if "M34" in line:
            return bullet_index["DIR_W"]
        # M15 direction based on N-S field
        if "M15" in line:
            if ns_direction == "N":
                return bullet_index["DIR_N"]  # Uptown - down arrow
            else:
                return bullet_index["DIR_S"]  # Downtown - up arrow
        # Fallback to MTA icon
        return bullet_index["MTA"]
    ...
    def update_display(self, arrival_data, bullet_alert_flag, now):
        ...
        # Combine all arrivals from both directions
        all_arrivals = []
        for arr in arrival_data.get("North", []):
            arr["_ns"] = "N"
            all_arrivals.append(arr)
        for arr in arrival_data.get("South", []):
            arr["_ns"] = "S"
            all_arrivals.append(arr)

        # Sort by arrival time
        all_arrivals = sorted(all_arrivals, key=lambda x: x["Arrival"])

        rows = min(self.default_rows, len(all_arrivals))
        ...
        for i in range(rows):
            self.arrivals_queue[i]["Line"] = all_arrivals[i]["Line"]
            self.arrivals_queue[i]["Arrival"] = all_arrivals[i]["Arrival"]
            self.arrivals_queue[i]["_ns"] = all_arrivals[i]["_ns"]
        ...
        for row, bus in enumerate(self.arrivals_queue):
            if row >= rows:
                break
            ...
            if row == 0:
                ...
                arrival_label_1.color = color[4]  # White

            elif row == 1:
                arrival_label_2.text = arrival_time
                arrival_label_2.color = color[4]  # White

            # Set the direction icon
            icon = self.get_bus_direction_icon(bus["Line"], bus.get("_ns", "S"))

            if bus["ALERT"] is True and bullet_alert_flag is True:
                if bus["FLASH"]:
                    if (now >= bus["PREV_TIME"] + bus["FLASH_OFF"]):
                        self.prev_time = now
                        mta_bullets[0,row] = bullet_index["ALERT"]
                        self.alert_flash = False

                elif bus["FLASH"] is False:
                    if (now >= bus["PREV_TIME"] + bus["FLASH_ON"]):
                        self.prev_time = now
                        mta_bullets[0,row] = icon
